...
<section class="m-t-0 p-t-0 p-b-100 m-b-100">
  ...
  $(document).ready(function() {
      var t = $("#mytable").DataTable({
          ...
          bInfo: false,
          lengthChange: false,
          ajax: {"url": '<?=base_url("public/notifikasi/json")?>', "type": "POST"},
          columns: [
              {
                  "data": "id_notif",
                  "orderable": false,
              ...
              },
              {
                ...
                {
                  if (row.status=="belum") {
                    return '<b><a href="<?=base_url()?>notifikasi/detail/'+row.id_notif+'" id="detail_notif">'+data+'</a></b>';
                  }else {
                    return '<a href="<?=base_url()?>notifikasi/detail/'+row.id_notif+'" id="detail_notif">'+data+'</a>';
                  }
                }
              },
              {
                ...
                {
                  if (row.status=="belum") {
                    return '<b><a href="<?=base_url()?>notifikasi/detail/'+row.id_notif+'" id="detail_notif">'+data.substr(0, 20)+'...</a></b>';
                  }else {
                    return '<a href="<?=base_url()?>notifikasi/detail/'+row.id_notif+'" id="detail_notif">'+data.substr(0, 20)+'...</a>';
                  }
                }
              },
          ],
