...
<a id="set-lokasi" href="<?=base_url()?>maps/<?=date('dmyhis')?>/" class="btn btn-block btn-lg btn-flat btn-info text-white" > Set lokasi saat ini</a>

</section>

<input type="hidden" id="kordinat" value="-5.131808242309933/119.43357472485354">

<script type="text/javascript">

  $(document).on('click',"#set-lokasi",function(e){
    e.preventDefault();
    window.location.href = $(this).attr('href')+$("#kordinat").val();
  })

  function getUrl()
  {
    window.location.href = "<?=base_url()?>maps/<?=tgl()?>/"+$("#kordinat").val();
  }
</script>
...
$(document).ready(function(){
  ...
  window.setInterval(function () {
            // initSubmitMap(_latitude,_longitude);
  }, 10000);
})
